#1.Đọc dữ liệu => numpy array
#2.Chọn K => tạo K tâm

#Lặp:
#   duyệt tất cả record: với mỗi record dữ liệu, tìm tâm gần nó nhất => label tâm cho nó
#   duyệt tất cả tâm: với mỗi tâm, lấy các record dữ liệu có label nó, gán tâm mới bằng trung bình tất cả các record
#lặp đến khi không còn thay đổi, hoặc đạt số lần lặp cụ thể
...
def init_K(dim, K):
    return 10*np.random.random((K,dim))-5
...
if __name__ == "__main__":
    #X = read_data("inputKmean.txt")
    
    X = random_data(1000,2)
    K = 6
